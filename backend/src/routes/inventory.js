...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const [docs, total] = await Promise.all([
      Model.find(filter).populate('category vendor').sort(sort).limit(+limit).skip((+page-1)*+limit),
...
router.get('/:id', protect, async (req, res) => {
  try {
    const company = req.user.company?._id || req.user.company
    const doc = await Model.findOne({ _id:req.params.id, company }).populate('category vendor')
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    await Audit.create({ company, user:req.user._id, userName:req.user.name, action:'CREATE', module:'Product', resource:'Product', resourceId:doc._id, message:`Created Product: ${doc.name||doc.title||doc.number||doc._id}` })
...
router.put('/:id', protect, async (req, res) => {
  try {
    ...
    await Audit.create({ company, user:req.user._id, userName:req.user.name, action:'UPDATE', module:'Product', resource:'Product', resourceId:doc._id, changes:{ before:before.toObject(), after:doc.toObject() } })
    res.json({ success:true, data:doc })
  } catch(err) { res.status(500).json({ success:false, message:err.message }) }
})
...
router.delete('/:id', protect, authorize('superadmin','admin','manager'), async (req, res) => {
  try {
    ...
    await Audit.create({ company, user:req.user._id, userName:req.user.name, action:'DELETE', module:'Product', resource:'Product', resourceId:doc._id })
